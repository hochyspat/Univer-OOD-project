...
public class UserService {
    ...
    public MessageOutputData registerUser(String name, String age, String height,
        ...
        if (!(isValidName(name))) {
            return new MessageOutputData(new InvalidParameterError("имя")
                    .getErrorMessage(), chatId);
        }
        if (!(isValidInputParameter(height, LOWER_HEIGHT_LIMIT, UPPER_HEIGHT_LIMIT))) {
            return new MessageOutputData(new InvalidParameterError("рост")
                    .getErrorMessage(), chatId);
        }
        if (!(isValidInputParameter(weight, LOWER_WEIGHT_LIMIT, UPPER_WEIGHT_LIMIT))) {
            return new MessageOutputData(new InvalidParameterError("вес")
                    .getErrorMessage(), chatId);
        }
        if (!(isValidInputParameter(age, LOWER_AGE_LIMIT, UPPER_AGE_LIMIT))) {
            return new MessageOutputData(new InvalidParameterError("возраст")
                    .getErrorMessage(), chatId);
        }

        if (!userRepository.existsById(chatId)) {
            User user = new User(name, Integer.parseInt(height), Integer.parseInt(weight),
